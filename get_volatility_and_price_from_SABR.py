...
class Calculator:
    ...
    def get_volatility(self, strike, time, params):
        volatility = ql.sabrVolatility(float(strike), self.spot, time, *params)
        return volatility
...
if __name__ == '__main__':
    ...
    params = []
    p1 = df1['alpha'].dropna().values
    p2 = df1['beta'].dropna().values
    p3 = df1['nu'].dropna().values
    p4 = df1['rho'].dropna().values
    for i in range(len(p1)):
        params.append([p1[i], p2[i], p3[i], p4[i]])
    ...
    if date < today or date > pd.to_datetime(dates[-1], format='%Y-%m-%d %H:%M:%S'):
        ...
        volatility = o.get_volatility(strike, time, params[index])
